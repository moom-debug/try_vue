...
export default {
    ...
    methods:{
        search(){
            ...
            this.$store.commit('REFRESH',{first:false,loading:true,users:[],err:''})
            axios.get(`https://api.github.com/search/users?q=${this.value}`).then(
                response=>{
                    ...
                    this.$store.commit('REFRESH',{first:false,loading:false,users:response.data.items,err:''})
                    console.log(this.$store)
                },
                error=>{
                    // this.$bus.$emit('user',{loading:false,user:[],err:'出错啊'})
                    this.$store.commit('REFRESH',{loading:false,user:[],err:'出错啊'})
                }
            )
        }
    }
